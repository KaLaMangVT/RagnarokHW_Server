	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (sc) {
			...
			if (sc->getSCE(SC_GLORIA))
				bonus += sc->getSCE(SC_GLORIA)->val1 * 50;
#endif
		}
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (sc) {
			...
			if (sc->getSCE(SC_MAGNIFICAT))
				bonus += sc->getSCE(SC_MAGNIFICAT)->val1 * 10;

			if (sc->getSCE(SC_GLORIA))
				bonus += sc->getSCE(SC_GLORIA)->val1 * 10;
#endif
		}
	} else if (type == STATUS_BONUS_RATE) {
		status_change *sc = status_get_sc(bl);
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, status_change *sc)
{
	...
	if (sc->getSCE(SC_ANGELUS))
		regen->rate.sp += sc->getSCE(SC_ANGELUS)->val1 * 5;

	if (sc->getSCE(SC_MAGNIFICAT))
		regen->rate.sp += sc->getSCE(SC_MAGNIFICAT)->val1 * 20;
...
int status_get_clan_id(struct block_list* bl)
{
	nullpo_ret(bl);
	switch (bl->type) {
	case BL_PC:
		return ((TBL_PC*)bl)->status.clan_id;
	case BL_PET:
		if (((TBL_PET*)bl)->master)
			return ((TBL_PET*)bl)->master->status.clan_id;
		break;
	case BL_MOB:
	{
		map_session_data* msd;
		struct mob_data* md = (struct mob_data*)bl;
		//if (md->guardian_data)	// Guardian's guild [Skotlex]
		//	return md->guardian_data->clan_id;
		if (md->special_state.ai && (msd = map_id2sd(md->master_id)) != NULL)
			return msd->status.clan_id; // Alchemist's mobs [Skotlex]
	}
	break;
	case BL_HOM:
		if (((TBL_HOM*)bl)->master)
			return ((TBL_HOM*)bl)->master->status.clan_id;
		break;
	case BL_MER:
		if (((TBL_MER*)bl)->master)
			return ((TBL_MER*)bl)->master->status.clan_id;
		break;
	/*case BL_NPC:
		if (((TBL_NPC*)bl)->subtype == NPCTYPE_SCRIPT)
			return ((TBL_NPC*)bl)->u.scr.clan_id;
		break;*/
	case BL_SKILL:
		if (((TBL_SKILL*)bl)->group)
			return ((TBL_SKILL*)bl)->group->clan_id;
		break;
	case BL_ELEM:
		if (((TBL_ELEM*)bl)->master)
			return ((TBL_ELEM*)bl)->master->status.clan_id;
		break;
	}
	return 0;
}
