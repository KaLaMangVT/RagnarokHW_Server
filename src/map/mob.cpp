...
static void mob_item_drop(struct mob_data* md, struct item_drop_list* dlist, struct item_drop* ditem, int loot, int drop_rate, bool flag)
{
	...
	test_autoloot = true; // [Fix] true autoloot
	/*test_autoloot = sd
...
int mob_dead(struct mob_data* md, struct block_list* src, int type)
{
	...
	} // [Fix] End loop save player list to attack this monster
	...
	if (!(type & 2) && //No exp
		...
		for (i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if ((temp = tmpsd[i]->status.party_id) > 0) {
				...
				if (j == pnum) { //Possibly add party.
					...
					if (pt[pnum].p) { // [Fix] Delete Condition Party Share
						pt[pnum].id = temp;
...
#ifdef RENEWAL
			if (job_exp && tmpsd[i] && tmpsd[i]->hd)
				hom_gainexp(tmpsd[i]->hd, job_exp * battle_config.homunculus_exp_gain / 100); // Homunculus only receive 10% of EXP [Fix]
#else
		...
		} // [Tag] End Player list attack to monster
	...
	{ // Item Drop [Fix]
		...
		int pnum = 0;
		std::map<int, int> markplayerroll;
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			pnum = 0;
			// [Fix] Start Loop all player drop
			markplayerroll.clear();
			for (int k = 0; k < DAMAGELOG_SIZE && md->dmglog[k].id; k++) {
				int flag = 1;

				if (!tmpsd[k]) continue;

				if ((temp = tmpsd[k]->status.party_id) > 0) {
					int j;
					for (j = 0; j < pnum && pt[j].id != temp; j++); //Locate party.
				...
				ShowStatus("N DEALER ADD: %d/%d\n", k, md->dmglog[k].id);
			} // [Tag] End Player list attack to monster


			// Drop Setting
			struct item_drop_list* dlist_custom = ers_alloc(item_drop_list_ers, struct item_drop_list);
			int drop_modifier_custom = 100;

			for (int k = 0; k < pnum; k++) //Party share.
			{
				map_session_data* sd[MAX_PARTY];
				unsigned int h, c, g;

				for (h = c = 0; h < MAX_PARTY; h++) {
					if ((sd[c] = pt[k].p->data[h].sd) == NULL || sd[c]->bl.m != src->m || pc_isdead(sd[c]))
						continue;
					c++;
				}
				...
				for (g = 0; g < c; g++) {
#ifdef RENEWAL_DROP
					drop_modifier_custom = pc_level_penalty_mod(map_charid2sd(sd[g]->status.char_id) != nullptr ? map_charid2sd(sd[g]->status.char_id) : nullptr, PENALTY_DROP, nullptr, md);
#endif

					dlist_custom->m = md->bl.m;
					dlist_custom->x = md->bl.x;
					dlist_custom->y = md->bl.y;
					dlist_custom->first_charid = sd[g]->status.char_id;
					dlist_custom->item = NULL;

					drop_rate = mob_getdroprate(src, md->db, md->db->dropitem[i].rate, drop_modifier, md);

					//if (markplayerroll.find(sd[g]->status.char_id) != markplayerroll.end()) continue;
					ShowStatus("N PARTY ADD: %d/%d\n", g, sd[g]->status.char_id);

					bool isDrop = rnd() % 10000 >= drop_rate;
					bool isMVPtoEGG = mvp_sd && it->type == IT_PETEGG;
					//bool isNotAttacker = mvp_sd->status.char_id != sd[g]->status.char_id;
					if (!isDrop && !isMVPtoEGG)
					{
						ditem = mob_setdropitem(&md->db->dropitem[i], 1, md->mob_id);
						mob_item_drop(md, dlist_custom, ditem, 0, battle_config.autoloot_adjust ? drop_rate : md->db->dropitem[i].rate, homkillonly || merckillonly);
					}

					// MVP DROP ###################
					if (md->db->dropitem[i+1].nameid == 0) // Check Last Loop Of Normal Item Drop
					{
						int temp = 0;
						if (mvp_sd && md->get_bosstype() == BOSSTYPE_MVP) {
							if (!(map_getmapflag(m, MF_NOMVPLOOT) || type & 1)) {
								...
								if (battle_config.item_drop_mvp_mode == 1) {
									//Random order
									for (int e = 0; e < MAX_MVP_DROP_TOTAL; e++) {
										while (1) {
											uint8 va = rnd() % MAX_MVP_DROP_TOTAL;
											if (mdrop[va].nameid == 0) {
												if (md->db->mvpitem[e].nameid > 0)
													memcpy(&mdrop[va], &md->db->mvpitem[e], sizeof(mdrop[va]));
												break;
											}
										}
									}
									...
									for (int e = 0; e < MAX_MVP_DROP_TOTAL; e++) {
										if (md->db->mvpitem[e].nameid > 0)
											memcpy(&mdrop[e], &md->db->mvpitem[e], sizeof(mdrop[e]));
									}
								}
								...
								for (int e = 0; e < MAX_MVP_DROP_TOTAL; e++) {
									...
									std::shared_ptr<item_data> i_data = item_db.find(mdrop[e].nameid);
									...
									drop_rate = mob_getdroprate(src, md->db, md->db->dropitem[e].rate, drop_modifier, md);

									//if (markplayerroll.find(sd[g]->status.char_id) != markplayerroll.end()) continue;
									ShowStatus("N PARTY MVP ADD: %d/%d\n", g, sd[g]->status.char_id);

									isDrop = rnd() % 10000 >= temp;
									isMVPtoEGG = mvp_sd && it->type == IT_PETEGG;
									//bool isNotAttacker = mvp_sd->status.char_id != sd[g]->status.char_id;
									if (!isDrop && !isMVPtoEGG)
									{
										struct item item = {};
										item.nameid = mdrop[e].nameid;
										item.identify = itemdb_isidentified(item.nameid);
										clif_mvp_item(sd[g], item.nameid);
										clif_mvp_effect(sd[g]);

										if ((temp = pc_additem(sd[g], &item, 1, LOG_TYPE_PICKDROP_PLAYER)) != 0) {
											clif_additem(sd[g], 0, 0, temp);
...
//
//			int temp_rate = 0;
//			std::map<int, int> playergetitemlist;
//
//			memset(&mdrop, 0, sizeof(mdrop));
//
//			//struct item_drop_list* dlist = ers_alloc(item_drop_list_ers, struct item_drop_list);
...
//					for (g = 0; g < c; g++) {
//
//						ShowStatus("PARTY ADD MVP: %d/%d\n", g, sd[g]->status.char_id);
//
//#if defined(RENEWAL_DROP)
//						int penalty = pc_level_penalty_mod(sd[g], PENALTY_MVP_DROP, nullptr, md);
//#endif
//
//						if (mdrop[i].nameid == 0)
//							continue;
...
//							continue;
//
//						drop_rate = mdrop[i].rate;
//
//#if defined(RENEWAL_DROP)
//						drop_rate = cap_value(apply_rate(drop_rate, penalty), 0, 10000);
//#endif
//
//						if (drop_rate != 10000) {
//							ShowStatus("temp != 10000\n");
//							if (drop_rate <= 0 && !battle_config.drop_rate0item)
//								drop_rate = 1;
//							if (rnd() % 10000 >= drop_rate) //if ==0, then it doesn't drop
//								continue;
//							ShowStatus("temp != 10000 > CON\n");
//						}
//
//						struct item item = {};
...
//							char message[128];
//							sprintf(message, msg_txt(NULL, 541), sd[g]->status.name, md->name, i_data->ename.c_str(), drop_rate / 100.);
...
//
//						if ((drop_rate = pc_additem(sd[g], &item, 1, LOG_TYPE_PICKDROP_PLAYER)) != 0) {
//							clif_additem(sd[g], 0, 0, drop_rate);
//							//map_addflooritem(&item, 1, mvp_sd->bl.m, mvp_sd->bl.x, mvp_sd->bl.y, mvp_sd->status.char_id, (second_sd ? second_sd->status.char_id : 0), (third_sd ? third_sd->status.char_id : 0), 1, 0, true);
//						}
//
//						ShowStatus("..\n");
//						if (i_data->flag.broadcast)
//							intif_broadcast_obtain_special_item(sd[g], item.nameid, md->mob_id, ITEMOBTAIN_TYPE_MONSTER_ITEM);
//
...
//			}
//
//			/*ShowStatus("Delete MAP\n");
//			for (auto const& pair : playergetitemlist) {
//				std::cout << pair.first << ": " << pair.second << std::endl;
//			}*/
//			ShowStatus("End Drop\n");
//			playergetitemlist.clear();
//		}
//	}
