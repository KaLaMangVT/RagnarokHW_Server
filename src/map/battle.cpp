...
#ifdef RENEWAL
		case KN_BRANDISHSPEAR:
			skillratio += 100 + 70 * skill_lv + sstatus->str * 3;
			break;
#else
		case KN_BRANDISHSPEAR:
#endif
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Check rivalry settings.
		...
		if (flag & BCT_CLAN || state & BCT_ENEMY)
		{
			int s_clan = status_get_clan_id(s_bl);
			int t_clan = status_get_clan_id(t_bl);
			if (s_clan && t_clan && (s_clan == t_clan))
				state |= BCT_CLAN;
		}
